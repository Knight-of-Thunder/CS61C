...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
            ...
            tmp = _mm_and_si128(cmp, tmp);
            sum = _mm_add_epi32(tmp, sum);
        }
        _mm_storeu_si128((__m128i *)tmpResultArray, sum);
        for(int i = 0; i < 4; i++){
            result += tmpResultArray[i];
        }
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {    clock_t start = clock();
